...
class CelloNodeImpl implements CelloNode {
  ...
  releaseRelayReservation(peerIdStr: string): boolean {
    ...
    const relay = (this.#libp2p.services as Record<string, unknown>)["relay"] as
      | {
          reservations?: {
            has(p: typeof peerId): boolean;
            get(p: typeof peerId): { signal?: { abort?: () => void } } | undefined;
            delete(p: typeof peerId): unknown;
          };
        }
      | undefined;
    ...
    /**
     * ⚠️ **ABORT THE EXPIRY SIGNAL, NOT JUST THE MAP ENTRY.** libp2p keeps a per-reservation timer
     * whose abort listener deletes the entry, and deleting the entry does not cancel the timer. Left
     * running, it fires later and deletes whatever reservation that peer holds AT THAT MOMENT — so
     * a peer that reconnects and reserves again silently loses the NEW slot mid-TTL, and its client
     * does not notice until it refreshes against an expiry that no longer exists.
     */
    try {
...
export async function createNode(opts: CreateNodeOptions): Promise<CelloNode> {
  ...
  const libp2p = await createLibp2p({
    ...
    transports: [
      tcp(),
      webSockets(),
      circuitRelayTransport(hasCircuitListen ? { reservationConcurrency: circuitListenCount } : {}),
    ],
