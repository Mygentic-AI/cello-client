...
export const ADJUDICATED: AdjudicatedClaim[] = [
   ...
   * `shippedSurfaces()` enumerates `.md` files plus one hard-coded string — it cannot produce that
   * surface. So the orphan-surface guard in `dod-m15-claim-scanner-1.test.ts` failed and **main went
   * red.** That guard is correct and caught exactly what it exists for.
   *
   * **`cello-mcp.ts` cannot be adjudicated here until the scanner can SEE it**, which is
   * `DOD-M15-TOOLDESC-SCAN-1`'s subject, not TIERTEXT's. Adding a second row-array consumed only by
   * TIERTEXT's own test would be the value-with-no-reader defect a third time, so the adjudications
   * live in the DoD entry until that line lands and they can be moved here with a baseline.
   */
  {
