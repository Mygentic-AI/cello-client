...
import { retirePrePostsChannelLog } from "./channel-log-upgrade.js";
import {
...
export class ChannelLogStore {
  ...
  constructor(db: DaemonDatabase, logger: Logger) {
    this.#db = db;
    this.#logger = logger;
    // 042-UPGRADE Part B: a channel_log that predates the post_cbor column cannot be written (its
    // leftover NOT-NULL columns reject every current insert), so it is retired aside BEFORE the
    // CREATE — see channel-log-upgrade.ts. The CREATE then makes the current shape. Kept in a
    // sibling module so this file mentions none of the retired schema (m16-016 removal guard).
