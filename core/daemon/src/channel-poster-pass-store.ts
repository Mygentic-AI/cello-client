...
export class ChannelPosterPassStore {
  ...
  put(agentId: string, channelHex: string, pass: HeldPosterPass): boolean {
    ...
    this.#db
      .prepare(
        `INSERT INTO channel_poster_passes (agent_id, channel_pubkey, pass_cbor, issued_at, expires_at)
         VALUES (?, ?, ?, ?, ?)
      ...
      .run(agentId, channelHex, Buffer.from(pass.pass_cbor), pass.issued_at, pass.expires_at);
  ...
  get(agentId: string, channelHex: string): HeldPosterPass | null {
    const row = this.#db
      .prepare(`SELECT pass_cbor, issued_at, expires_at FROM channel_poster_passes WHERE agent_id = ? AND channel_pubkey = ?`)
