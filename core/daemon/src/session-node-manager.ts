 ...
 */
import { ABUSE_MAX_UNKNOWN_SESSIONS_GLOBAL, type AbandonNoticeResult, type ActiveSessionEntry, type AwaitingAckEntry, CONTENT_MAX_INBOUND_STREAMS, type CreateSessionResult, type ISessionNodeFactory, LEAF_FETCH_GRACE_MS, type ParkedDrainReason, type QuarantinedRecord, type ReceivedContentEntry, type RefusalNotice, type RelayConnectParams, SHUTDOWN_STEP_DEADLINE_MS, type SessionImpairment, type SessionRevivalIdentity, type TranscriptEntry, type WitnessAlertNotice } from "./session-node-types.js";
...
holdOwnLeafForTest(agentName: string, sessionId: string, canonicalSeq: number, contentHashHex: string): void { return this.#held.holdOwnLeafForTest(agentName, sessionId, canonicalSeq, contentHashHex); }
  ...
  constructor(opts: {
    ...
    this.#seal = new SessionSeal({
      ...
      destroySessionSeed: (a, sid) => this.#destroySessionSeed(a, sid),
      updateSessionStatus: (a, sid, status, by) => this.#updateSessionStatus(a, sid, status, by),
    });
    ...
    this.#relay = new SessionRelay({
      ...
      markInterruptedWithDetails: (a, sid, n, source) => this.markInterruptedWithDetails(a, sid, n, source),
    });
  ...
  async createSessionNode(
    ...
    if (relay) {
      await this.#relay.connectSessionRelay(sessionId, node, agentName, relay, correlationId);
    }
  ...
  async connectToCounterparty(
    ...
    await this.#relay.authorizeCircuitDialsToCounterparty(agentName, sessionId, entry, addrs);
    let lastError = "";
    for (const addr of addrs) {
      try {
  ...
  async reviveSessionNode(
    ...
    const reservations = this.#relay.reservationCircuitAddrs(agentName);
    ...
    await this.#relay.reconnectRevivedSessionRelay(agentName, sessionId, node, gater, correlationId, persistedRelay);
    ...
    if (!this.#updateSessionStatus(agentName, sessionId, "active")) {
      ...
      if (revivedEntry?.node === node) this.#relay.detachSessionRelay(revivedEntry);
