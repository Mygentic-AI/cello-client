...
holdOwnLeafForTest(agentName: string, sessionId: string, canonicalSeq: number, contentHashHex: string): void { return this.#held.holdOwnLeafForTest(agentName, sessionId, canonicalSeq, contentHashHex); }
  ...
  /** DOD-M15-NOTACCEPTING-1 — shut a tier or re-open it; is it shut; who was turned away. See SessionRecords. */
  ...
  async initialize(): Promise<void> {
    ...
    this.#logger.info("persist.db.opened", { encrypted: true });
  ...
  checkUnknownSenderAcceptanceBound(
    ...
    // `DOD-M15-NOTACCEPTING-1` D2c — a SHUT tier is refused before any counting, so the operator's
    // mark bites at the GATE rather than only choosing wording (`SessionRecords.isTierNotAccepting`
    // carries the why). The reason stays the CAPACITY code, so the refusal VALUE never separates
    // shut from full; the caller's MESSAGE is chosen in `inbound-sessions.ts`.
