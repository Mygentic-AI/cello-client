...
export class SessionNodeManager {
  ...
  readQuarantined(agentName: string, sessionId: string, sequence?: number): QuarantinedRecord[] {
    ...
    const rows = this.#db
      ...
      .all(...(sequence === undefined
        ...
        : [this.#requireAgentId(agentName), sessionId, sequence])) as Array<{
  ...
  async ingestReceivedContent(
    ...
    if (!record) {
      ...
      /**
       * RETAINED, and this is the case the retention matters most for. A message for a session this
       * daemon has no record of is the least explicable thing that can arrive, so it is the thing an
       * operator has the least other way to show anyone. There is no `sessions` row and no
