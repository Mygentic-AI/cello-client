      ...
      if (remaining <= 0) {
        ...
        if (liveness === "gone") {
          ...
          return {
            ...
            // lands HERE, pre-empting the quiet exit. A wrong explanation plus an action is worse
            // than saying nothing at all.
            ...refusalsField(sessionNodeManager, agentName, sessionId, connectionId),
            reason: "counterparty_gone",
            liveness: "gone",
            /**
             * ⚠️ **NAMES WHAT WAS OBSERVED, AND `gone` IS A SMALLER FACT THAN IT SOUNDS.**
             *
             * The only thing that produced this state is `onPeerDisconnect` firing for the peer id
             * recorded as the counterparty's session peer — one libp2p connection went away. It
             * does not establish that they crashed, went offline, or left. The previous wording
             * said "it may have crashed or gone offline" and then told the operator to seal, which
             * is a network story for something that is very often a verification fault: a peer
             * whose messages this side keeps refusing is never acknowledged, drops the direct path,
             * and arrives here looking exactly like a crash.
             *
             * **And it must not lead with the seal.** A seal is irreversible and, taken on this
             * evidence, is precisely the truncated close DOD-M15-WITHHOLD-SEAL-1 exists to stop.
