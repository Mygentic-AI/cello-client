...
/**
 * ⚠️ `retryAfterMs` on the FAILURE member — `DOD-M15-RELAYABUSE-1`, review HIGH-2.
 *
 * The relay tells us exactly when a throttle clears, and until now only the LIVE send path could
 * hear it: the drain path returned `{parked:false, error}` and dropped the number on the floor. With
 * a backlog larger than one window that is the whole feature undone — the relay allows N per window,
 * the drain deposits N, every remaining item is refused carrying a fresh delay, and all of them fall
 * back to waiting for an unrelated reconnect. Which is precisely the condition the retry timer was
 * added to fix.
 *
 * It sits on the failure member for the same reason `sequenceNumber` does elsewhere: a refusal still
 * carries information the caller needs, and dropping it because the operation failed is how a
 * value ends up with no reader.
 */
export type ParkResult = { parked: true } | { parked: false; error: string; retryAfterMs?: number };
...
export class RetryQueue {
   ...
   */
  async drainAwaitingToPark(
    agentId: string,
    sessionId: string,
    parkFn: ParkFn,
    /**
     * DOD-M15-RELAYABUSE-1 review HIGH-2: called ONCE per pass with the largest retry delay any
     * refusal in that pass reported, or never if none did.
     *
     * ⚠️ Per PASS, not per item, and that is the whole point. A backlog larger than one rate-limit
     * window produces a refusal per remaining item, each carrying its own delay; scheduling on each
     * would be one timer per message — the flood the limiter exists to prevent, generated by the
     * code that respects it. The largest delay is the one that is actually true for the window.
     */
    onRetryAfter?: (retryAfterMs: number) => void,
  ): Promise<number> {
    const ak = this.#ak(agentId, sessionId);
    const q = this.#awaiting.get(ak);
    ...
    if (q.length === 0) this.#awaiting.delete(ak);
    // ONCE per pass — see the parameter's note on why this is not per item.
    if (maxRetryAfterMs > 0) onRetryAfter?.(maxRetryAfterMs);
    return parked;
  }
}
