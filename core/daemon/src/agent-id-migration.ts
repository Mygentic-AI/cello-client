...
const REKEY_TARGETS: readonly RekeyTarget[] = [
  ...
  },
  {
     ...
     * replays each ALTER-carrying table's inline ALTERs in the real boot order and asserts seeded
     * VALUES survive across all seven rebuilt tables, so a column this DDL omits shows up as a
     * failing test rather than as an empty column on an operator's machine after an upgrade. Keep
     * the two together — the list below and that replay are one guard in two halves.
     *
