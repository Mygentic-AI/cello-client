...
describe("RetryQueue", () => {
  ...
  describe("CELLO-M7-MSG-001 AC-005: awaiting-ACK content (park target)", () => {
    ...
    it("★★ DOD-M15-RELAYABUSE-1: a refused pass reports the relay's retry delay ONCE, not per item", async () => {
      /**
       * ⚠️ ONCE PER PASS IS THE WHOLE POINT, and per-item would be the defect.
       *
       * A backlog larger than one rate-limit window produces a refusal for EVERY remaining item,
       * each carrying its own delay. Scheduling on each would be one timer per message — the flood
       * the limiter exists to prevent, generated by the code that respects it.
       *
       * Before this, the drain path dropped the delay entirely, so with a backlog the relay allowed
       * what the window permitted and every remaining message fell back to waiting for an unrelated
       * reconnect — the exact condition the retry was added to remove.
       */
      ...
      const delays: number[] = [];
      const parked = await retryQueue.drainAwaitingToPark(
        "alice",
        sessionId,
        async () => ({ parked: false, error: "rate_limited", retryAfterMs: 45_000 }),
        (ms) => delays.push(ms),
      );

      expect(parked, "nothing parked — every item was refused").toBe(0);
      expect(
        delays.length,
        "FIVE refusals must produce ONE report, not five. Per-item would schedule five timers for " +
          "one window, which is the flood this control exists to stop.",
      ).toBe(1);
      expect(delays[0], "and it must be the delay the relay actually asked for").toBe(45_000);
      expect(retryQueue.getAwaitingDepth("alice", sessionId), "and nothing is lost").toBe(5);
    });

    it("★★ the LARGEST delay in a pass wins, and a pass with none reports nothing", async () => {
      /**
       * The largest is the one that is actually true for the window: retrying at the smallest means
       * arriving early and being refused again, which converts a polite shortcut into extra load.
       * And a pass whose refusals carry no delay must schedule nothing at all — an absent value is
       * not a zero, and the event triggers remain the guarantee.
       */
      ...
      const seen: number[] = [];
      let n = 0;
      await retryQueue.drainAwaitingToPark(
        "alice",
        sessionId,
        async () => {
          n += 1;
          return { parked: false, error: "rate_limited", retryAfterMs: n * 1_000 };
        },
        (ms) => seen.push(ms),
      );
      expect(seen, "the largest delay seen in the pass").toEqual([3_000]);

      const none: number[] = [];
      await retryQueue.drainAwaitingToPark(
        "alice",
        "sess-maxdelay",
        async () => ({ parked: false, error: "relay_down" }),
        (ms) => none.push(ms),
      );
      expect(none, "a refusal with no delay must schedule nothing — absent is not zero").toEqual([]);
    });
