...
describe("M8C-TGDOOR-1: Telegram doorbell", () => {
  ...
  it("G7: TGDOOR is inert (no sends) until configured", async () => {
    ...
    await snm.ingestReceivedContent("alice", "aa".repeat(16), new TextEncoder().encode("hi"), msgLeafHash(new TextEncoder().encode("hi")), "c1", undefined, "sha256");
    await wait(30);
  ...
  it("G3/G4: message-waiting is coalesced (one ring per unread period); reading clears it for the NEXT message", async () => {
    ...
    await snm.ingestReceivedContent("alice", sid, new TextEncoder().encode("m1"), msgLeafHash(new TextEncoder().encode("m1")), "c1", undefined, "sha256");
    await snm.ingestReceivedContent("alice", sid, new TextEncoder().encode("m2"), msgLeafHash(new TextEncoder().encode("m2")), "c2", undefined, "sha256");
    await wait(30);
    ...
    await snm.ingestReceivedContent("alice", sid, new TextEncoder().encode("m3"), msgLeafHash(new TextEncoder().encode("m3")), "c3", undefined, "sha256");
    await wait(30);
  ...
  it("G3 (per-session isolation): two different sessions each ring independently, one does not suppress the other", async () => {
    ...
    await snm.ingestReceivedContent("alice", sidA, new TextEncoder().encode("a1"), msgLeafHash(new TextEncoder().encode("a1")), "ca1", undefined, "sha256");
    await snm.ingestReceivedContent("alice", sidB, new TextEncoder().encode("b1"), msgLeafHash(new TextEncoder().encode("b1")), "cb1", undefined, "sha256");
    await wait(30);
    ...
    await snm.ingestReceivedContent("alice", sidA, new TextEncoder().encode("a2"), msgLeafHash(new TextEncoder().encode("a2")), "ca2", undefined, "sha256");
    await wait(30);
